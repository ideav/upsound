...
В шаблоне могут присутствовать точки вставки вида `{_global_.xxx}`, которые шаблонизатор заменяет своими внутренними переменными, например:
`{_global_.z}` - эти имя текущей БД (инстанса) Интеграма
`{_global_.uid}` - ID текущего пользователя
...
Из шаблона рабочего места надо убирать всё, что уже есть в `main.html`, и оставить только специфичные для него вещи.
...
**Важно:** Используйте `window.db` в скриптах, генерирующих запросы.
...
`JSON` возвращает отчет с информацией о его колонках. В этом случае ключ data содержит массивы значений для каждой колонки: сколько колонок, столько массивов.
В массиве перечислены значения колонки с соответствующим номером, например:
```
{
    "columns": [
        {
...
Переменная `window.xsrf` проинициализирована на странице и её можно использовать.

### Создание и изменение записи

Команда `_m_new` должна иметь параметр `up` - это ID родителя. Если в задании не указано иное, то передавать up=1, то есть создаваемая запись не подчинена никому и находится в корне.
