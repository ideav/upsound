...
При изменении скриптов и стилей обязательно добавляйте параметр версии (`ver` или номер PR) к URL, чтобы браузер загрузил актуальную версию файла, а не использовал кэшированную:
...
<!-- Вариант 2: Параметр ver -->
<script src="/download/{_global_.z}/info.js?ver=5"></script>
<link rel="stylesheet" href="/download/{_global_.z}/info.css?ver=5">
```

**Правило:** При каждом изменении файла увеличивайте номер версии или используйте номер текущего PR.
...
Чтобы API вернул результат в формате JSON в запрос должен быть добавлен параметр `JSON_KV` для команд `report/` и `JSON` для всех остальных команд.
